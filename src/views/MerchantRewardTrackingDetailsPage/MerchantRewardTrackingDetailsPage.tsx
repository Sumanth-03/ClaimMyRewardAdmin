...
const CampaignRewardTrackingDetailsPage = () => {
    ...
    const [requestDetails, setrequestDetails] =
        useState<MerchantRewardTrackingData>()
    const tableData = useAppSelectorTable(
        (state) => state.merchantRewardTracking.data.TrackingList,
    )
    const chatDetails = useAppSelector(
        (state) => state.merchantRewardTrackingDetails.data.message,
    )
    ...
    function findObjectById(
        Id: String,
        array: MerchantRewardTrackingData[],
    ): MerchantRewardTrackingData | undefined {
    ...
    const handlePostMessage = async (message: String, Id?: String) => {
        ...
        try {
            await dispatch(
                postMessage({ ticket_id: Number(Id), msg: message, type: 1 }),
            )
        } catch (error) {
            ...
            <section className="flex flex-col md:flex-row w-full bg-white shadow-lg rounded-lg p-6 mb-6 0 min-h-[500px]">
                <div className=" w-full md:w-[40%] ">
                    <div className="flex flex-col justify-evenly items-center border-r border-gray-200 p-4 max-h-80">
                        ...
                        <p className="text-gray-600 flex gap-2 my-1">
                        ...
                        <p className="text-gray-600 flex gap-2 items-center">
                            <IoMail size={16} /> {requestDetails?.user_email}
                        </p>
                    </div>
                    <div className="flex flex-col justify-between p-4 space-y-4 ">
                        <h1 className="text-2xl font-bold text-gray-900">
                            {requestDetails?.retailer}
                        </h1>
                        <h2 className="text-lg font-medium text-gray-700">
                            Order Id: {requestDetails?.order_id}
                        </h2>
                        <p className="text-gray-600 text-justify">
                            Order Amount: {requestDetails?.order_amt}
                        </p>
                        <p className="text-lg font-semibold ">
                            Reward Amount:{' '}
                            <span className="text-green-600">
                                ₹ {requestDetails?.reward_amt}
                            </span>
                        </p>
                        <aside className="flex flex-col bg-white shadow-md rounded-lg mt-5 p-2 ">
                            <h2 className="text-lg w-full font-medium text-center p-2">
                            ...
                            <div className="w-full mx-auto  bg-white shadow-sm rounded-lg">
                                ...
                                >
                                    ...
                                    />
                                    <button className="mt-4 w-fit px-5 bg-primary ml-auto text-white py-2 rounded-lg hover:bg-primary">
                                        Process Now
                                    </button>
                                </div>
                            </div>
                ...
                <section className="w-full md:w-[60%] max-h-fit rounded-xl mx-auto  bg-white shadow-md">
                    ...
                    <div
                        ref={chatBoxRef}
                        className="h-96 overflow-y-auto px-10 scrollbar-thin scrollbar-thumb-gray-600"
                    >
                        {alteredChat?.map((chat) => (
                            <div
                                key={chat?.created_at}
                                className={`mb-4 ${chat?.actor === 1 ? 'text-right ' : 'text-left'}`}
                            >
                                ...
                                <div
                                    className={`chatBox relative inline-block p-2 rounded-lg ${chat?.actor === 1 ? 'bg-blue-100 chatBox_admin' : 'bg-gray-200 chatBox_customer'}`}
                                >
